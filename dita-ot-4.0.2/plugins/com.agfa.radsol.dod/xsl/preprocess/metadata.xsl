<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 
  ...
  <!-- metadata.xsl output is saved to metadata.xml, which is outside of DITA-OT
       used by browsefile.xslt to generate navigation -->
  
  <xsl:param name="argsInput"/><!-- path of the exported ditamap -->
  ...
  <xsl:template match="/">
    ...
    <!-- using the exported ditamap, not the one in the dita-ot temp folder, because the topic
         files are renamed and not traceable to the original -->
    <xsl:variable name="ditamap" select="document($argsInput)"/>
  ...
  <xsl:template match="topicref" mode="ditamap">
    <topicref>
      ...
      <xsl:variable name="authoringtimestamp">
        <xsl:variable name="t" select="substring-after($topicfilename, '_')"/>
        <xsl:value-of select="substring-before($t, '.xml')"/>
      </xsl:variable>
      ...
      <authoringrevision><xsl:value-of select="$authoringtimestamp"/></authoringrevision>
      <reviseddate><xsl:value-of select="$authoringtimestamp"/></reviseddate>
