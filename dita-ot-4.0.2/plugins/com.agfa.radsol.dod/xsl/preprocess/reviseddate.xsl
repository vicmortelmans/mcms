    ...
    <xsl:choose>
      <xsl:when test="contains($original-ditamap-path, '/published/')">
        ...
        <xsl:call-template name="decode">
          <!-- the encoding is done in python using urrlib.parse.quote_plus() and
               hidden for XSLT's doc() function by replacing '%' by '~' -->
          <xsl:with-param name="str" select="replace(replace($version-encoded,'\+',' '),'~','%')"/>
        </xsl:call-template>
      </xsl:when>
      <xsl:otherwise>
